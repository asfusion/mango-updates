<cfsilent>
	...
	<cfif NOT len(panel)>
		...
		<cfelseif listfind(currentAuthor.currentRole.permissions, "manage_all_posts")>
	...
	<cfelse>
		<cfset posts = postsManager.getPostsByCustomField("adminCustomPanel",panel, (page * itemsPerPage) + 1, itemsPerPage,true,false) />
		<cfset pageTitle = "#panelData.label#: All" />
		<!--- since I can't get the total, I'll make another query for the first next item,
			if there is one, then there are more pages --->
		<cfset hasNextPage = arraylen(postsManager.getPostsByCustomField("adminCustomPanel",panel, ((page + 1) * itemsPerPage) + 1, 1,true,false)) />
	</cfif>
