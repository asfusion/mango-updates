...
<cfset recordsFrom = attributes.from />
	...
	<cfswitch expression="#source#">
		<cfcase value="recent">
			<cfset posts = request.blogManager.getPostsManager().getPosts(recordsFrom,attributes.count)/>		
		</cfcase>
		<cfcase value="category,multicategory">
			<cftry>
			<cfset posts = request.blogManager.getPostsManager().getPostsByCategory(attributes.categoryName,recordsFrom,attributes.count)/>
