...
<cf_layout page="Themes" title="Themes">
	<div id="wrapper">
		<cfif listfind(currentAuthor.currentRole.permissions, "manage_themes")
				OR listfind(currentAuthor.currentRole.permissions, "set_themes")>
...
var skinUpdate#skins[i].id# = new Spry.Data.XMLDataSet("datasets/downloadableSkins.cfm?action=checkUpdates&skin=#skins[i].id#", "skin");
	...
	<div spry:region="skinUpdate#skins[i].id#">
		...
		<div spry:state="ready" spry:repeat="skinUpdate#skins[i].id#">
			<p class="message" spry:if="'{hasupdates}' == 1">There is an update available for this theme. <a href="{downloadUrl}">Download</a></p></div>
	</div>
	</td></tr>
	</cfif>
</cfloop>  
...
<p class="message">Your role does not allow editing themes</p>
</div></div>
</cfif>
</div>
</cf_layout>