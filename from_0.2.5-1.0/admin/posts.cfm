<cfsilent>
	...
	<!--- get posts --->
	<cfif NOT len(panel)>
		<cfif listfind(currentAuthor.currentRole.permissions, "manage_all_posts")>
			<cfset posts = request.administrator.getRecentPosts() />
		<cfelse>
			<cfset posts = request.blogManager.getPostsManager().getPostsByAuthor(currentAuthor.id,true) />
		</cfif>
	<cfelse>
		<cfset posts = request.blogManager.getPostsManager().getPostsByCustomField("adminCustomPanel",panel,1,0,true,false) />
...
<div id="wrapper">
<cfif listfind(currentAuthor.currentRole.permissions, "manage_all_posts") OR
		listfind(currentAuthor.currentRole.permissions, "manage_posts")>
	<div id="submenucontainer">
		<ul id="submenu">
			...
			<mangoAdmin:CustomMenu name="posts" menu="secondary" owner="#panel#">
	...
	<div id="content">
		...
		<table  cellspacing="0">
			<tr><th class="buttonColumn">Edit</th><th>Title</th><th>Date</th><th>Status</th><th>Comments</th><th>Delete</th></tr>
			<cfloop from="1" to="#arraylen(posts)#" index="i">
				...
				<tr>
					...
					<td <cfif NOT i mod 2>class="alternate"</cfif>>#posts[i].getTitle()#</td>
					...
					<td <cfif NOT i mod 2>class="alternate"</cfif>><a href="posts.cfm?action=delete&amp;id=#posts[i].getId()#"  class="deleteButton">Delete</a></td>
				</tr>
				</cfif>
			</cfloop>
		</table>
...
<p class="message">Your role does not allow you to edit posts</p>
</div></div>
</cfif>
</div>
</cf_layout>