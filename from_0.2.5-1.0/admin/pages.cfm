<cfsilent>
	...
	<!--- get posts --->
	<cfif NOT len(panel)>
		<cfif listfind(currentAuthor.currentRole.permissions, "manage_all_pages")>
			<cfset posts = request.administrator.getPages() />
		<cfelse>
			<cfset posts = pagesManager.getPagesByAuthor(currentAuthor.id,true) />
		</cfif>
	<cfelse>		
		<cfset posts = pagesManager.getPagesByCustomField("adminCustomPanel",panel,1,0,true,false) />
...
<cfif listfind(currentAuthor.currentRole.permissions, "manage_all_pages") OR
		listfind(currentAuthor.currentRole.permissions, "manage_pages")>
		...
		<table cellspacing="0">
			...
			<cfloop from="1" to="#arraylen(posts)#" index="i">
				...
				<tr>
					...
					<td <cfif NOT i mod 2>class="alternate"</cfif>><a href="pages.cfm?action=delete&amp;id=#posts[i].getId()#"  class="deleteButton">Delete</a></td>
				</tr>
				</cfif>
			</cfloop>
		</table>
...
<p class="message">Your role does not allow you to edit pages</p>
</div></div>
</cfif>
</div>
</cf_layout>