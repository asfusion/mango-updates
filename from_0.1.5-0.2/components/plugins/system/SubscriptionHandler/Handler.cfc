			...
			<cfswitch expression="#arguments.event.getName()#">
				...
				<cfcase value="afterCommentAdd">
					...
					<!--- also check whether this comment has been approved --->
						<cfif comment.getApproved() AND (NOT len(isImport) OR NOT isImport)>
						...
						<cfset emailTemplate = replacenocase(emailTemplate,"{postLink}",postlink ,"all") /><!--- @TODO fix this, possibly by getting the post from manager instead of factory--->
						...
						<cfset subject = replacenocase(subject,"{postLink}",postlink ,"all") /><!--- @TODO fix this, possibly by getting the post from manager instead of factory--->
						...
						<cfset body = replacenocase(body,"{postLink}", postlink ,"all") /><!--- @TODO fix this, possibly by getting the post from manager instead of factory--->
						...
						<cfif postAuthorEmail NEQ comment.getCreatorEmail()>
							<cfset mail = structnew() />
							<cfset mail.from = comment.getCreatorEmail() />
					...
					<cftry>
						<cfif structkeyexists(session,"author")>
							<cfset variables.manager.getAdministrator().pluginUpdated(
									"SubscriptionsHandler", variables.id, session.author) />
						</cfif>
