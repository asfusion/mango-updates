<cfcomponent>
	...
	<cffunction name="processEvent" hint="Synchronous event handling" access="public" output="false" returntype="any">
		<cfargument name="event" type="any" required="true" />
			...
			<!--- admin nav event --->
			<cfelseif eventName EQ "linksNav">
			...
			<!--- admin event --->
			<!---  showLinksSettings ----------------------------------------------- --->
			<cfelseif eventName EQ "links-showLinksSettings">
			...
			<!---  editLinkSettings ----------------------------------------------- --->
			<cfelseif eventName EQ "links-editLinkSettings">
				<cfset data = arguments.event.getData() />
				<cfset categories = getCategories(0) />
			...
			<cfelseif eventName EQ "links-editLinkCategorySettings">
				<cfset data = arguments.event.getData() />
				<cfset dao = createObject("component","LinksDAO").init(variables.manager.getQueryInterface()) />
			...
			<!---  deleteCategory ----------------------------------------------- --->
			<cfelseif eventName EQ "links-deleteLinkCategory">
				<cfset data = arguments.event.getData() />
				<cfset dao = createObject("component","LinksDAO").init(variables.manager.getQueryInterface()) />
			...
			<!---  deleteLink ----------------------------------------------- --->
			<cfelseif eventName EQ "links-deleteLink">
				<cfset data = arguments.event.getData() />
				<cfset dao = createObject("component","LinksDAO").init(variables.manager.getQueryInterface()) />
...
<cffunction name="getCategories" access="private" output="false" returntype="array">
	<cfargument name="cache" type="numeric" required="false" default="60" />
		...
		<cfsavecontent variable="myQueryString">
			<cfoutput>SELECT * FROM #prefix#link_category</cfoutput>
		</cfsavecontent>
