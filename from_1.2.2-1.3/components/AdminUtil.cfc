...
<!--- ::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::: --->
	<cffunction name="getAuthors" access="public" output="false" returntype="array">
		<cfset var authorsManager = variables.blogManager.getauthorsManager() />
		<cfset var authors =  authorsManager.getAuthorsByBlog(variables.blogManager.getBlog().getId(), true) />
...
<!--- ::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::: --->
	<cffunction name="editAuthor" access="public" output="false" returntype="struct">		
		...
		<cfargument name="user" required="false" type="any">
			...
			<!--- set the role for the current blog --->
...
<!--- ::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::: --->
	<cffunction name="copyPluginAssets" access="private" output="false" returntype="any">
		...
		<cfargument name="pluginId" type="string" required="true" />
			...
			<cfset var pluginPath = blog.getSetting("pluginsDir") & "user/" & arguments.plugin />
			...
			<cftry>
				...
				<cfcatch type="any">
					<!--- log the error --->
					<cfset local.logger = createObject("component","utilities.Logger") />
					<cfset local.logger.setLevel("warning") />
					<cfset local.logger.logObject("warning",cfcatch,"Blog #id#: Error while copying assets for plugin #arguments.plugin#") />
				</cfcatch>
			</cftry>
		...
		<cfargument name="pluginId" type="string" required="true" />
			...
			<cftry>
				...
				<cfcatch type="any">
					...
					<cfset local.logger = createObject("component","utilities.Logger") />
					<cfset local.logger.setLevel("warning") />
					<cfset local.logger.logObject("warning",cfcatch,"Blog #id#: Error while deleting assets for plugin #arguments.plugin#") />
				</cfcatch>
			</cftry>
...
<!--- ::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::: --->
	<cffunction name="activatePlugin" access="public" output="false" returntype="any">
		...
		<cfargument name="user" required="false" type="any">
			...
			<cfset var result = structnew()/>
			<cfset var id = blog.getId()/>
			<cfset var pluginPrefs = createObject("component","utilities.PreferencesFile").init(dir)>
			...
			<cfif NOT listfind(currentList,plugin)>
				...
				<cfif pluginQueue.pluginExists(arguments.pluginId)>
					...
					<cftry>
						<cfset local.pluginResult = pluginObj.setup()/>
						...
						<cfset copyPluginAssets(arguments.plugin,arguments.pluginId) />
...
<!--- ::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::: --->
	<cffunction name="getCustomPanels" access="public" output="false" returntype="struct">
		<cfargument name="cached" default="true" type="boolean" />
		
		<cfset var queues = structnew() />
		<cfset var panel = "" />
		<cfset var adminSettings = variables.blogManager.getBlog().getSetting('admin') />
		<cfset var xmlFiles = "" />
		<cfset var panelXmlFile = "" />
		<cfset var panelData = "" />
		
		<cfif NOT structkeyexists(variables,"adminCustomPanels") OR NOT arguments.cached>
			<cfset queues["post"] = createobject("component","utilities.Queue") />
			<cfset queues["page"] = createobject("component","utilities.Queue") />
			<cfset variables.panelIds = structnew() />
			
			<cfdirectory name="xmlFiles" directory="#adminSettings.customPanels.directory#" action="list" filter="*.xml" recurse="false">
			
			<cfoutput query="xmlFiles">
				<cfset panelXmlFile = directory & "/" & name />
				<cfif fileexists(panelXmlFile)>
					<cftry>
					<cffile action="read" file="#panelXmlFile#" variable="panelData">
					<cfset panel = createobject("component","model.AdminCustomPanel").init() />
					<cfset panel.initFromXML(panelData) />
					<!--- add the path to the template info --->
					<cfif len(panel.template)>
						<cfset panel.template = adminSettings.customPanels.path & panel.template />
					</cfif>
					<cfset queues[panel.entryType].addElement(panel,panel.order) />
					<cfset variables.panelIds[panel.id] = panel />
					<cfcatch type="any"></cfcatch>
					</cftry>
				</cfif>
			</cfoutput>
			<cfset variables.adminCustomPanels = structnew() />
			<cfset variables.adminCustomPanels["posts"] = queues["post"].getElements() />
			<cfset variables.adminCustomPanels["pages"] = queues["page"].getElements() />
		</cfif>
		
		<cfreturn variables.adminCustomPanels />
	</cffunction>
...
<!--- ::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::: --->
	<cffunction name="getCustomPanel" access="public" output="false" returntype="any">
		...
		<cfif structkeyexists(variables.panelIds,arguments.id)>
			<cfreturn variables.panelIds[arguments.id] />
		<cfelse>
