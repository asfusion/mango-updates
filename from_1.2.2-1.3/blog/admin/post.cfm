<cfsilent>
	...
	<cftry>
		<cfif mode EQ "update">
			...
			<cfif post.status NEQ "published">
			...
			<cfset pagetitle = 'Editing "<a href="#postUrl#">#post.getTitle()#</a>"'>
		<cfelse>
			<cfset post = request.blogManager.getObjectFactory().createPost() />
		</cfif>	
		
		<cfif post.customFieldExists("adminCustomPanel")>
			<cfset panel = post.getCustomField("adminCustomPanel").value />		
		</cfif>
	<cfcatch type="any">
		...
		<cfif mode EQ "new">
			...
			<cfif listfind(currentAuthor.currentRole.permissions, "publish_posts")>
	...
	<cfif NOT listfind(currentAuthor.currentRole.permissions, "publish_posts") AND listfind(showFields, "status")>
...
<cfif listfind(currentAuthor.currentRole.permissions, "manage_all_posts") OR
		(listfind(currentAuthor.currentRole.permissions, "manage_posts") AND 
...
<p class="message">Your role does not allow <cfif mode EQ "new">adding new posts<cfelse>editing this post</cfif></p>
</div></div>
</cfif>
</div>
</cf_layout>