...
--->
	<cfscript>
		this.name = "mango_#right(hash(GetDirectoryFromPath(GetCurrentTemplatePath())),50)#_v1_2_2";
	...
	<cffunction name="onRequestStart" returnType="boolean" output="true">
		<cfargument type="String" name="targetPage" required="true" />
			...
			<cfif structkeyexists(url,"logout")>
				<cfset request.blogManager.removeCurrentUser() />
			</cfif>
			
			<cfif structkeyexists(form,"login")>
				<cfset isAuthor = request.blogManager.getAuthorsManager().attemptLogin(form.username,form.password) />
				<cfif NOT isAuthor>
			...
			<cfif isAdmin AND NOT request.blogManager.isCurrentUserLoggedIn()>
